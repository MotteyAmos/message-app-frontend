...
export default function MessageInputBox({ roomId }: { roomId: string }) {
  ...
  const send = async () => {
    try {
      ...
      console.log("roomId in MessageInputBox:", roomId);
      if (!userInput) return;
      await createMessage({
        variables: {
          messageInput: {
  ...
  return (
    <div className="bg-accent z-40 flex  bottom-5 w-[90%] px-2 py-0 pb-2 rounded-md   min-h-20 max-h-132 items-center overflow-hidden ">
      ...
      <textarea
        className=" flex-1 focus-visible:ring-0 focus-visible:ring-offset-0 shadoow-none outline-none min-h-15 max-h-132"
        onChange={handleInputChange}
      ></textarea>
      <div
        className="flex self-end gap-3 h-full  justify-center  cursor-pointer "
        onClick={send}
      >
        <Paperclip size={20} />
        <GrEmoji size={20} />
